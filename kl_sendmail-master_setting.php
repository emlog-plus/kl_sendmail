...
jQuery(function($){
	$('#port').onlyPressNum();
	$('#testsend').click(function(){$('#testresult').html('邮件发送中..');$.get('../content/plugins/kl_sendmail-master/kl_sendmail-master_test_do.php',{sid:Math.random()},function(result){if($.trim(result)!=''){$('#testresult').html(result);}else{$('#testresult').html('发送失败！');}});});
...
<form id="form1" name="form1" method="post" action="plugin.php?plugin=kl_sendmail-master&action=setting">
<div class="row">
<div class="col-sm-12">
...
<input type="password" name="password" value="<?php echo KL_MAIL_PASSWORD;?>" class="form-control" />
  ...
  <div class="form-group">
    <label>收信邮箱</label>
<input name="toemail" type="text" id="toemail" class="form-control" value="<?php echo KL_MAIL_TOEMAIL;?>"/>
  </div> 
  <div class="form-group">
    <label>发送方式</label>
...
{
	//修改配置信息
	$fso = fopen(EMLOG_ROOT.'/content/plugins/kl_sendmail-master/kl_sendmail-master_config.php','r'); //获取配置文件内容
	$config = fread($fso,filesize(EMLOG_ROOT.'/content/plugins/kl_sendmail-master/kl_sendmail-master_config.php'));
	fclose($fso);
	...
	$fso = fopen(EMLOG_ROOT.'/content/plugins/kl_sendmail-master/kl_sendmail-master_config.php','w'); //写入替换后的配置文件
